...
\subsection{Kristof}
Heeft enkele pagina's gemaakt en ook de back-end verzorgd die nodig was voor die pagina's. Heeft de basis gelegd voor user profiles en heeft private usergroups gemaakt. Bouwde een notificatiesysteem en zorgde ook voor de email reminders. Zorgde voor de setup en onderhoud van onlineversie.
\subsection{Tom}
...
Facebook login is met \'e\'en gedachte in het achterhoofd gebouwd, namelijk ervoor zorgen dat mensen die een account op Facebook geen saaie registratie velden hoeven in te vullen voor de duizendste keer maar direct binnen kunnen. We willen dus dat Facebook gebruikers zichzelf snel kunnen inloggen en dat is dan ook mogelijk door bij het inloggen niet op de login knop te drukken maar op de blauwe Facebook Login knop. Vervolgens worden er wat administratieve zaken met Facebook afgehandeld waarna de gebruiker op de homepage terechtkomt als een geregistreerde coachcenter gebruiker. Merk op dat, wegens beperkingen van de Facebook plugin, Facebooklogin enkel beschikbaar is vanaf coachcenter.be (z\'onder www.!)'. \\ \\
...
In de lijst van vereiste functionaliteit stond ook dat er voor ingelogde gebruikers de mogelijkheid moest zijn om groepen van gebruikers te maken die tegen elkaar kunnen spelen.
...
\\
\\
Bovenaan de usergroup zien we ook de leden met hun betscore ernaast. Zo ontstaat er een soort van competitiedrang die het aantal bets in het systeem alleen maar zal opdrijven. Merk op dat er geen andere bets kunnen worden uitgebracht binnen een usergroup. Als een gebruiker bet op een match, dan geldt deze bet binnen alle usergroups. Zo dwingen we onze gebruikers om goed na te denken over elke bet.
...
\section{Appendix}
 ...
 \textit{Er wordt dagelijks een mail gestuurd voor de matchen binnen de 24 uur en wekelijks voor de matchen binnen een week}
...
Volledige lijst van onze queries  te vinden in ./queries.sql \\
