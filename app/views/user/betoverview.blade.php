		...
		@foreach ($futureBets as $bet)
			<div class="panel panel-primary" style="margin-top:10px;">
				...
				<div class="panel-body">
					<p><a href="{{ route('team', array('hometeam'=>urlencode($futureBetsMatches[$matchIndex]->hometeam_id))) }}">{{$futureBetsMatches[$matchIndex]->hometeam}}</a>: <?php if ($bet->hometeam_reds != NULL)echo $bet->hometeam_reds; else echo "/"; ?><img style="height:16px; margin-bottom:7px;" src="{{asset('img/redcard.png')}}"/> <?php if ($bet->hometeam_yellows != NULL)echo $bet->hometeam_yellows; else echo "/"; ?><img style="height:16px; margin-bottom:7px;" src="{{asset('img/yellowcard.png')}}"/></p>
					<p><a href="{{ route('team', array('awayteam'=>urlencode($futureBetsMatches[$matchIndex]->awayteam_id))) }}">{{$futureBetsMatches[$matchIndex]->awayteam}}</a>: <?php if ($bet->awayteam_reds != NULL)echo $bet->awayteam_reds; else echo "/"; ?><img style="height:16px; margin-bottom:7px;" src="{{asset('img/redcard.png')}}"/> <?php if ($bet->awayteam_yellows != NULL)echo $bet->awayteam_yellows; else echo "/"; ?><img style="height:16px; margin-bottom:7px;" src="{{asset('img/yellowcard.png')}}"/></p>
      	...
      	@foreach ($pastBets as $bet)
			<div class="panel panel-primary" style="margin-top:10px;">
				...
				<div class="panel-body">
					...
					<p><a href="{{ route('team', array('awayteam'=>urlencode($pastBetsMatches[$matchIndex]->awayteam_id))) }}">{{$pastBetsMatches[$matchIndex]->awayteam}}</a>: <?php if ($bet->awayteam_reds != NULL)echo $bet->awayteam_reds; else echo "/"; ?><img style="height:16px; margin-bottom:7px;" src="{{asset('img/redcard.png')}}"/> <?php if ($bet->awayteam_yellows != NULL)echo $bet->awayteam_yellows; else echo "/"; ?><img style="height:16px; margin-bottom:7px;" src="{{asset('img/yellowcard.png')}}"/></p>
					<p><i class="fa fa-circle"></i> First Goal:  <?php if ($bet->first_goal == 0) echo "/";
